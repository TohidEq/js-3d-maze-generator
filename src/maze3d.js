class Maze3D {
  constructor(width, height, depth) {
    ...
    this.depth = depth + (depth % 2 == 0); // convert to odd if it is not
    this.grid = this.initGrid();
    // Choose a random position to start  (1, (x,y,z)-2) BCZ first and last cells should be wall and cannot be choosen as started position
    this.xStart = this.randOdd(1, this.width - 2);
    this.yStart = this.randOdd(1, this.height - 2);
    this.zStart = this.randOdd(1, this.depth - 2);
  }
    ...
    for (let z = 0; z < this.depth; z++) {
      ...
      for (let y = 0; y < this.height; y++) {
        ...
        for (let x = 0; x < this.width; x++) {
          row.push(true); // wall (filling x1 x2 x3 x4 x... xn)
        }
        depth.push(row); // (filling y1 y2 y3 y4 y... yn)
      }
      grid.push(depth); // (filling z1 z2 z3 z4 z... zn)
    }
    return grid; // result: grid[z][y][x]
  }
  ...
  getNeighbors(x, y, z, canBeCraved = false, onlyCraved = false) {
    // -- neighbors[x][y][z]
    const neighbors = [];
    // -- directions[x][y][z]
    const directions = [
    ...
    for (const [dx, dy, dz] of directions) {
      ...
      let condition = false;
      if (onlyCraved) {
        if (this.inBounds(nx, ny, nz) && !this.grid[nz][ny][nx])
          condition = true;
      } else if (canBeCraved) {
        if (this.inBounds(nx, ny, nz) && this.grid[nz][ny][nx])
          condition = true;
      } else if (this.inBounds(nx, ny, nz) && this.grid[nz][ny][nx])
        condition = true;
      if (condition) {
  ...
  carvePath(x1, y1, z1, x2, y2, z2) {
    const mx = (x1 + x2) >>> 1;
    const my = (y1 + y2) >>> 1;
    const mz = (z1 + z2) >>> 1;
    this.grid[z1][y1][x1] = false; // path
    this.grid[mz][my][mx] = false; // path between
    this.grid[z2][y2][x2] = false; // path
  }
  ...
  generateDFS(
    ...
    this.grid[startZ][startY][startX] = false;
    ...
    while (stack.length > 0) {
      ...
      if (neighbors.length === 0) {
        ...
        const [nx, ny, nz] = neighbors[randomNeighborsIndex];
        // Remove wall between current cell and choosen one
        this.carvePath(x, y, z, nx, ny, nz);
  ...
  generateAldousBroder(
    ...
    const allUnvisitedCellsCount =
      ((this.width - 1) / 2) * ((this.height - 1) / 2) * ((this.depth - 1) / 2);

    let visitedCellsCount = 1;

    let [x, y, z] = [startX, startY, startZ];

    while (visitedCellsCount + 2 != allUnvisitedCellsCount) {
      const neighbors = this.getNeighbors(x, y, z, true, false);
      // if (neighbors.length === 0) continue;

      console.log(startZ, startY, startX);
      console.log(neighbors);
      const randomNeighborsIndex = ~~(Math.random() * neighbors.length);
      const [nx, ny, nz] = neighbors[randomNeighborsIndex];

      // if (this.grid[nz][ny][nx]) {
      this.carvePath(x, y, z, nx, ny, nz);
      visitedCellsCount++;
      // }
  ...
  generatePrime(
    ...
    const neighbors = this.getNeighbors(startX, startY, startZ);
    neighbors.map((wall) => {
    ...
    while (walledNeighbors.length != 0) {
      ...
      const [wx, wy, wz] = walledNeighbors[randomNeighborsIndex];

      const cravedNeighbors = this.getNeighbors(wx, wy, wz, true, true);

      if (cravedNeighbors.length > 0) {
        this.grid[wz][wy][wx] = false;

        const randomCravedNeighborsIndex = ~~(
          Math.random() * cravedNeighbors.length
        );
        const [cnx, cny, cnz] = cravedNeighbors[randomCravedNeighborsIndex];

        this.carvePath(wx, wy, wz, cnx, cny, cnz);

        // Get and Add new walls to walledNeighbors
        const newWalls = this.getNeighbors(wx, wy, wz);
